...
A **transactional outbox** stores the business change and a pending publication record in the same database transaction. A relay later publishes that record to Kafka. This preserves the obligation to publish after a database commit, but relay retries can duplicate events, so consumers still need safe deduplication.
