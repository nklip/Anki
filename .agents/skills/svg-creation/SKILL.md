...
The checker validates local URL references in attributes, inline styles, and stylesheets, including quoted targets. It estimates transformed text runs and nested `<tspan>` bounds with shared text anchors, samples elliptical connector arcs, rejects embedded raster artwork, and compares a supplied PNG using contrast from the reference background. Treat `possible text/connector overlap`, ink-recall, ink-precision, pixel-error, ink-delta, and content-bounds warnings as actionable.

Geometry uses simple tag/class CSS rules and normal horizontal text flow; it does not implement a browser's complete cascade, font shaping, or per-character positioning. Unsupported text coordinates produce warnings, and curve sampling is bounded. Exact font metrics, arrowhead-to-boundary clearance, complex backgrounds, and semantic correctness still require full-size rendered inspection.
